...
import 'package:goshops/game/models/board.dart';
import 'package:goshops/game/models/tile.dart';
...
class GameBloc extends Bloc<GameEvent, GameState> {
  ...
  GameBloc() : super(const GameState()) {
    ...
    on<ChangePosition>(changePosition);
  }
  ...
  onKey(event, emit) {
    ...
    if (event.event.isKeyPressed(LogicalKeyboardKey.arrowRight)) {
      direction = SwipeDirection.right;
    } else if (event.event.isKeyPressed(LogicalKeyboardKey.arrowLeft)) {
      direction = SwipeDirection.left;
    } else if (event.event.isKeyPressed(LogicalKeyboardKey.arrowUp)) {
      direction = SwipeDirection.up;
    } else if (event.event.isKeyPressed(LogicalKeyboardKey.arrowDown)) {
  ...
  newGame(event, emit) {
    emit(state.copyWith(
        board: Board.newGame(
            state.board?.best ?? 0 + (state.board?.score ?? 0), [random([])])));
  }
