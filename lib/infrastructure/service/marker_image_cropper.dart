...
class ImageCropperForMarker {
  Future<BitmapDescriptor> resizeAndCircle(String? imageURL, num size) async {
    final File imageFile = await urlToFile(imageURL);
    final Uint8List byteData = imageFile.readAsBytesSync();
    final Image image = await _resizeAndConvertImage(byteData, size.toInt(), size.toInt());
  ...
  Canvas _performCircleCrop(Image image, Size size, Canvas canvas) {
    ...
    canvas.drawRRect(
      ...
      Paint()
        ..color =
            (LocalStorage.getMode() ? CustomStyle.white : CustomStyle.bgDark)
                .withOpacity(0.75)
